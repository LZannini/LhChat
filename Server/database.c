...
int update_password(char *username, char *nuova_pass){
  PGconn *conn = conentti(CONN_STRING);
  PGresult *res;
  char query[1024], error[1024];
  int out = 0;
...
int update_password(char *username, char *nuovo_user){
  PGconn *conn = conentti(CONN_STRING);
  PGresult *res;
  char query[1024], error[1024];
  int out = 0;
  ...
  if (conn != NULL){
    sprintf(query, "update utente set username = $$%s$$ where username = $$%s$$", nuova_pass, username);
    res = PQexec(conn, query);
    strcpy(error, PQresultErrorMessage(res));
    if(strlen(error) > 0){
      printf("%s\n", error);
...
int check_if_admin(char *username, int id_stanza){
    PGconn *conn = connetti(CONN_STRING);
    PGresult *res;
    char query[1024], error[1024];
